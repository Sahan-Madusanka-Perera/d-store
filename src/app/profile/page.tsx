        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card className="border-0 shadow-sm">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold text-foreground">{profile?.full_name || 'Valued User'}</div>
                        <p className="text-sm text-muted-foreground mt-1">Joined {new Date(profile?.created_at || user.created_at).toLocaleDateString()}</p>
                    </CardContent>
                </Card>
                ...
                <Card className="border-0 shadow-sm">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold text-foreground">{recentOrders.length > 0 ? recentOrders.length : 0}</div>
                ...
                <CardContent className="p-0">
                    {error ? (
                        <div className="p-8 text-center">
                            ...
                            <h3 className="text-lg font-medium text-foreground mb-1">Database Setup Required</h3>
                        ...
                        <div className="p-8 text-center border-t border-border">
                            <div className="mx-auto w-12 h-12 bg-muted text-muted-foreground/70 rounded-full flex items-center justify-center mb-4">
                                <Package className="h-6 w-6" />
                            </div>
                            ...
                            <Link href="/products" className="inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-primary-foreground bg-primary hover:bg-primary/90">
                                Start Shopping
                            </Link>
                        </div>
                    ) : (
                        <div className="divide-y divide-gray-100">
                            {recentOrders.map((order) => (
                                <div key={order.id} className="p-6 hover:bg-muted transition-colors flex items-center justify-between">
