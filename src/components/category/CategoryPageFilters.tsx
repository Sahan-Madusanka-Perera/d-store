...
export default function CategoryPageFilters({ basePath, isMobile = false, extraFilters }: CategoryPageFiltersProps) {
  ...
  return (
    <div className={isMobile ? 'px-2 py-2' : 'bg-card rounded-2xl border border-border p-6 shadow-sm sticky top-24'}>
      <h3 className="font-bold text-xl text-foreground mb-6">Filters</h3>

      {/* Extra category-specific filters */}
      {extraFilters?.map((filter) => (
        <div key={filter.paramName} className="mb-8">
          <h4 className="font-medium text-sm text-muted-foreground uppercase tracking-wider mb-4">{filter.label}</h4>
            ...
            {filter.options.map((opt) => (
              <label key={opt.id} className="flex items-center gap-3 cursor-pointer group">
                <input
                  ...
                  className="w-4 h-4 text-foreground bg-background border-border focus:ring-ring focus:ring-offset-background"
                />
                <span className={`text-sm ${extraState[filter.paramName] === opt.id ? 'font-semibold text-foreground' : 'text-muted-foreground group-hover:text-foreground transition-colors'}`}>
                  {opt.name}
                </span>
              </label>
            ))}
          </div>
          <div className="h-px bg-border w-full mt-8"></div>
        </div>
      ))}
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground/70">Rs.</span>
            <input
              type="number"
              placeholder="Min"
              value={minPrice}
              onChange={(e) => setMinPrice(e.target.value)}
              className="w-full pl-8 pr-3 py-2 text-sm bg-background border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-ring/20"
            />
          </div>
        ...
        <Button onClick={applyCustomRange} className="w-full mt-4 bg-primary hover:bg-primary/90 text-primary-foreground" size="sm">
          Apply Custom Range
        </Button>
      </div>

      {hasActiveFilters && (
        <Button onClick={clearFilters} variant="outline" className="w-full text-muted-foreground hover:text-foreground border-border">
          Clear All Filters
        </Button>
      )}
    </div>
